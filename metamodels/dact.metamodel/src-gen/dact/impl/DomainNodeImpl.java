...
public abstract class DomainNodeImpl extends ExecutableNodeImpl implements DomainNode {
	...
	protected EClass eStaticClass() {
		return DactPackage.eINSTANCE.getDomainNode();
	}
	...
	public boolean RequiresMainExpression(final DiagnosticChain diagnostics, final Map<Object, Object> context) {
		...
		if (((!isValid) && (diagnostics != null))) {
			...
			this.report(diagnostics, 
				"RequiresMainExpression", _plus_3);
		}
		return isValid;
	}
	...
	public boolean SelfAnchoredBinaryExpression(final DiagnosticChain diagnostics, final Map<Object, Object> context) {
		...
		final boolean allowsAssignment = ((this instanceof DomainCreateAction) || (this instanceof DomainUpdateAction));
		EList<Expression> _mainExpr = this.getMainExpr();
		for (final Expression expr : _mainExpr) {
			{
				Identifier identifier = null;
				if ((expr instanceof BinaryExpression)) {
					Expression _leftExpr = ((BinaryExpression)expr).getLeftExpr();
					boolean _not = (!(_leftExpr instanceof UnaryExpression));
					if (_not) {
						String _name = null;
						if (((BinaryExpression)expr)!=null) {
							_name=((BinaryExpression)expr).getName();
						}
						String _plus = ("The left operand of expression \'" + _name);
						String _plus_1 = (_plus + 
							"\' must be a unary expression.");
						isValid = this.report(diagnostics, 
							"SelfAnchoredBinaryExpression", _plus_1);
					}
					else {
						Expression _leftExpr_1 = ((BinaryExpression)expr).getLeftExpr();
						identifier = ((UnaryExpression) _leftExpr_1).getIdentifier();
					}
				}
				else {
					if ((allowsAssignment && (expr instanceof VariableAssignment))) {
						identifier = ((VariableAssignment) expr).getIdentifier();
					}
					else {
						String _name_1 = null;
						if (expr!=null) {
							_name_1=expr.getName();
						}
						String _plus_2 = ("Expression \'" + _name_1);
						String _plus_3 = (_plus_2 + "\' of node \'");
						...
						if (allowsAssignment) {
							_xifexpression = " or an assignment.";
						}
						else {
							_xifexpression = ".";
						}
						String _plus_6 = (_plus_5 + _xifexpression);
						isValid = this.report(diagnostics, 
							"SelfAnchoredBinaryExpression", _plus_6);
					}
				}
				if (((identifier == null) || (!identifier.getName().startsWith("self")))) {
					...
					String _plus_7 = ("The left operand of expression \'" + _name_3);
					String _plus_8 = (_plus_7 + 
						"\' must be an identifier anchored at \'self\'.");
					isValid = this.report(diagnostics, 
						"SelfAnchoredBinaryExpression", _plus_8);
				}
			}
		}
		return isValid;
	...
	public int eDerivedOperationID(int baseOperationID, Class<?> baseClass) {
		if (baseClass == DiagnosticSupported.class) {
			switch (baseOperationID) {
				case DactPackage.DIAGNOSTIC_SUPPORTED___REPORT__DIAGNOSTICCHAIN_STRING_STRING: return DactPackage.DOMAIN_NODE___REPORT__DIAGNOSTICCHAIN_STRING_STRING;
				case DactPackage.DIAGNOSTIC_SUPPORTED___WARN__DIAGNOSTICCHAIN_STRING_STRING: return DactPackage.DOMAIN_NODE___WARN__DIAGNOSTICCHAIN_STRING_STRING;
				default: return -1;
			}
		}
	...
	public Object eInvoke(int operationID, EList<?> arguments) throws InvocationTargetException {
		switch (operationID) {
			...
			case DactPackage.DOMAIN_NODE___REPORT__DIAGNOSTICCHAIN_STRING_STRING:
				return report((DiagnosticChain)arguments.get(0), (String)arguments.get(1), (String)arguments.get(2));
			case DactPackage.DOMAIN_NODE___WARN__DIAGNOSTICCHAIN_STRING_STRING:
